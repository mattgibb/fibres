function centroids = generate_element_centroids_tarantula_method(pathstr,base_coords)
% Finds coordinates of centroids of elements


% constructs filename
filename = [pathstr '/image_renum'];
...
for i = 1:length(elements)
    ...
    centroids(i,1) = 0.25*(x1 + x2 + x3 + x4) - base_coords(1);
    centroids(i,2) = 0.25*(y1 + y2 + y3 + y4) - base_coords(2);
    centroids(i,3) = 0.25*(z1 + z2 + z3 + z4) - base_coords(3);
